 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`bcccore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`bcccore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'bcccore-' + name : 'bcccore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/bcccore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external bcccore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:bcccore-lib -o bcccore-lib.js';
    }
